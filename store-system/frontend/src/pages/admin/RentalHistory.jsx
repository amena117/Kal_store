...
import {
  ...
  Download, Eye, Clock, Users, Edit3, TrendingUp, DollarSign, User, FileText
} from 'lucide-react';
...
const countChanges = h => [
  parseFloat(h.old_total||0)   !== parseFloat(h.new_total||0),
  parseFloat(h.old_advance||0) !== parseFloat(h.new_advance||0),
  String(h.old_return_date||'') !== String(h.new_return_date||''),
      ...
      <div className="audit-drawer">
        <div className="audit-drawer-header">
          <div>
            ...
            <div style={{ fontSize:'0.75rem', color:'var(--text-muted)', marginTop:'0.15rem' }}>Rental #{record.rental_id}</div>
          </div>
        ...
        <div className="audit-drawer-body">
          ...
          <div className="audit-editor-strip">
            ...
            <div style={{ flex:1, minWidth:0 }}>
              ...
              <div style={{ fontSize:'0.75rem', color:'var(--text-muted)' }}>{fmtDate(record.date)} &middot; {fmtTime(record.date)}</div>
            </div>
          ...
          <div style={{ marginBottom:'1rem', padding:'0.75rem 1rem', background:'rgba(255,255,255,0.03)', borderRadius:'var(--radius-sm)', border:'1px solid var(--glass-border)' }}>
            <div style={{ fontSize:'0.7rem', fontWeight:700, color:'var(--text-muted)', textTransform:'uppercase', letterSpacing:'0.06em', marginBottom:'0.4rem' }}>Customer</div>
            <div style={{ fontWeight:600, display:'flex', alignItems:'center', gap:'0.4rem' }}>
              <User size={13} style={{ color:'var(--text-muted)' }}/> {record.rental_customer || 'Unknown'}
            </div>
          </div>

          {/* Field diffs */}
          <div className="audit-section-title"><Edit3 size={11}/> Field Changes</div>
          <DiffRow label="Total Payment"   oldVal={record.old_total}       newVal={record.new_total}       type="currency" icon={DollarSign}/>
          <DiffRow label="Advance Payment" oldVal={record.old_advance}     newVal={record.new_advance}     type="currency" icon={TrendingUp}/>
          <DiffRow label="Return Date"     oldVal={record.old_return_date} newVal={record.new_return_date} type="date"     icon={Calendar}/>
...
const RentalHistory = () => {
  ...
  const fetchHistory = async () => {
    try {
      setLoading(true);
      const res = await api.get('/rentals/history' + branchParam);
      setHistory(res.data || []);
  ...
  const exportData = () => exportToCSV(
    filtered.map(h => ({
      'Edit Date': h.date, 'Rental ID': h.rental_id, Customer: h.rental_customer,
      'Old Total': h.old_total, 'New Total': h.new_total,
      'Old Advance': h.old_advance, 'New Advance': h.new_advance,
      'Old Return Date': h.old_return_date, 'New Return Date': h.new_return_date,
  ...
  const STATS = [
    ...
    { icon: Users,      label: 'Rentals Touched', value: uniqueRecords,        color: '#10B981', bg: 'rgba(16,185,129,0.15)' },
    ...
    <div className="animate-fade-in-up" style={{ paddingBottom:'2rem' }}>
      ...
      <div className="page-header mb-4">
        <div>
          <h1 className="text-3xl font-bold" style={{ display:'flex', alignItems:'center', gap:'0.625rem' }}>
            <span style={{ background:'linear-gradient(135deg,#10B981,#059669)', borderRadius:'0.5rem', padding:'0.4rem 0.6rem', display:'inline-flex', alignItems:'center' }}>
              <Edit3 size={20} color="white"/>
            </span>
            Standalone Rental Audit
          </h1>
      ...
      <div className="glass-card mb-4 p-4">
        <div className="flex flex-wrap gap-3">
          <div className="relative flex-1" style={{ minWidth:'220px' }}>
            ...
            <input type="text" placeholder="Search customer, editor, rental ID..." className="form-control pl-9" value={search} onChange={e => setSearch(e.target.value)}/>
          </div>
      ...
      <div className="table-wrapper">
        <table className="glass-table">
          <thead>
            <tr>
              <th>Edit Date</th><th>Rental ID</th><th>Customer</th>
              <th>Total Payment</th><th>Advance</th><th>Return Date</th>
          ...
          <tbody>
            {loading ? (
              ...
              const totalChg  = parseFloat(h.old_total||0)   !== parseFloat(h.new_total||0);
              const advChg    = parseFloat(h.old_advance||0) !== parseFloat(h.new_advance||0);
              const dateChg   = String(h.old_return_date||'') !== String(h.new_return_date||'');
              const n         = countChanges(h);
              const totalDir  = parseFloat(h.new_total||0) >= parseFloat(h.old_total||0);
              const advDir    = parseFloat(h.new_advance||0) >= parseFloat(h.old_advance||0);
              return (
                <tr key={h.id} className={selected?.id === h.id ? 'audit-selected-row' : ''}>
                  ...
                  <td><span className="badge badge-success">#{h.rental_id}</span></td>
                  <td style={{ fontWeight:600 }}>{h.rental_customer || <span style={{ color:'var(--text-muted)', fontStyle:'italic' }}>Unknown</span>}</td>

                  <td>
                    {totalChg ? (
                      <div className="diff-values" style={{ gap:'0.3rem' }}>
                        <span className="change-pill change-pill-down">{fmtCurrency(h.old_total)}</span>
                        <ArrowRight size={10} style={{ color:'var(--text-muted)', flexShrink:0 }}/>
                        <span className={`change-pill ${totalDir ? 'change-pill-up' : 'change-pill-down'}`}>{fmtCurrency(h.new_total)}</span>
                      </div>
                    ) : <span style={{ color:'var(--text-muted)', fontSize:'0.8rem' }}>{fmtCurrency(h.new_total)}</span>}
                  </td>
                  ...
                  <td>
                    {advChg ? (
                      <div className="diff-values" style={{ gap:'0.3rem' }}>
                        ...
                        <ArrowRight size={10} style={{ color:'var(--text-muted)', flexShrink:0 }}/>
                        <span className={`change-pill ${advDir ? 'change-pill-up' : 'change-pill-down'}`}>{fmtCurrency(h.new_advance)}</span>
                      </div>
                  ...
                  <td>
                    {dateChg ? (
                      <div className="diff-values" style={{ gap:'0.3rem' }}>
                        <span className="change-pill change-pill-text" style={{ fontSize:'0.7rem' }}>{fmtDate(h.old_return_date)}</span>
                        <ArrowRight size={10} style={{ color:'var(--text-muted)', flexShrink:0 }}/>
                        <span className="change-pill change-pill-up" style={{ fontSize:'0.7rem' }}>{fmtDate(h.new_return_date)}</span>
                      </div>
                    ) : <span style={{ color:'var(--text-muted)', fontSize:'0.8rem' }}>{fmtDate(h.new_return_date)}</span>}
                  </td>
