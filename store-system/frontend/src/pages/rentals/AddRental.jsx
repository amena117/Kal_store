...
const AddRental = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [usersRes, memoryRes] = await Promise.all([
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      alert(err.response?.data?.message || "Failed to add rental");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="animate-fade-in-up pb-10 max-w-4xl mx-auto">
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <h1 className="text-3xl font-bold flex items-center gap-2">New Rental</h1>
          <p className="text-muted">Register a separate rental transaction</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="glass-card p-6">
          ...
          <datalist id="product-memory">
            {memoryProducts.map((p, idx) => <option key={idx} value={p} />)}
          </datalist>
          
          <datalist id="category-memory">
            ...
            {items.map((item, index) => (
              <div key={index} className="flex flex-col md:flex-row gap-3 items-center bg-glass-bg p-3 border border-glass-border rounded-lg relative group transition-all">
                <div className="flex-1 w-full">
                  ...
                  <input type="text" required list="product-memory" className="form-control" placeholder="e.g. Chair"
                ...
                <div className="flex-1 w-full">
                  ...
                  <input type="text" required list="category-memory" className="form-control" placeholder="e.g. Furniture"
        ...
        <div className="flex justify-end pt-4">
          <button type="submit" className="btn btn-primary" disabled={loading}>
            {loading ? 'Saving...' : <><Save size={18} /> Save Rental</>}
          </button>
        </div>
      </form>
    </div>
